...
export default function ElectronicBackground() {
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      concentricCircles.x = canvas.width * 0.85
      concentricCircles.y = canvas.height * 0.3
    }
    ...
    for (let i = 0; i < nodes.length; i++) {
      for (let j = i + 1; j < nodes.length; j++) {
        ...
        if (distance < 200 && Math.random() > 0.85) {
          connections.push({ from: i, to: j, progress: 0, active: Math.random() > 0.4 })
        }
      }
    }

    const concentricCircles = { x: canvas.width * 0.85, y: canvas.height * 0.7, rings: 8, maxRadius: 500, rotation: 0 }
    ...
    const animate = () => {
      ...
      connections.forEach((conn, index) => {
        ...
        const hue = 280 + Math.sin(time + index) * 30
        ctx.strokeStyle = conn.active ? `hsla(${hue}, 70%, 60%, 0.4)` : "rgba(120, 80, 150, 0.15)"
      ...
      if (Math.random() > 0.97) {
        ...
        particles.push({ x: sparkX, y: sparkY, vx: (Math.random() - 0.5) * 2, vy: (Math.random() - 0.5) * 2, life: 1, color: `hsl(${Math.random() * 60 + 10}, 80%, 60%)` })
      }
